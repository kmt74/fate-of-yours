  ...
  return (
    <div id="Selected-Cards-Tray" className="mx-auto flex w-full max-w-[1200px] flex-col gap-3 px-6 py-4">
      <div className="flex items-center gap-2.5">
        <span className="text-[0.72rem] tracking-[0.18em]" style={{ fontFamily: "'Cinzel', serif", color: selectedIds.length > 0 ? "#C9A84C" : "rgba(240,230,211,0.3)", transition: "color 0.3s" }}>SELECTED CARDS</span>
      ...
      <div className="flex justify-center gap-4">
        {POSITIONS.map((pos, i) => <SelectedSlot key={pos} position={pos} card={selectedCards[i] ?? null} onRemove={selectedCards[i] ? () => onRemoveCard(i) : undefined} />)}
      </div>
